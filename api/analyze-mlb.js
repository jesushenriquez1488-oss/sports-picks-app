...
function buildPlayerPropSeasonCoverage({
  logs = [],
  market,
  line,
  side
...
function calculateFinalPlayerPropSignal({
  ...
  /*
   * Solo usamos condiciones que
   * realmente tengan porcentaje.
   *
   * IMPORTANTE:
   * 0% SÍ es información válida.
   * null / undefined NO.
   */
  const availableHistorical =
    historicalCoverages
      .map(coverage =>
        coverage?.percentage
      )
   ...
   * histórica real, no fabricamos
   * confidence final.
   */
  if (!availableHistorical.length) {
    return null;
  }

  const historicalAverage =
    availableHistorical.reduce(
      (sum, value) =>
        sum + value,
      0
    ) /
  ...
  /*
   * 50% modelo CashEdge
   * 50% contexto histórico real.
   */
  const originalSideConfidence =
    (
      model * 0.50
    ) +
    (
  ...
  /*
   * La sumatoria completa decide
   * la dirección final.
   */
  if (normalizedOriginal >= 50) {
    return {
      side: originalSide,
...
const playerCache = new Map();
 ...
 const batterVsPitcherCache =
  new Map();
 const batterVsPitcherCareerCache =
  new Map();
...
const careerLogs =
  await getCachedPlayerCareerLogsOnly(
    playerInfo.id,
    playerInfo.primaryPosition === "P"
      ? "pitching"
      : "hitting"
...
let batterVsPitcherCareerCoverage = null;
...
let careerMatchupRows = [];

if (
  batterVsPitcherCareerCache.has(
    careerMatchupKey
  )
) {
  careerMatchupRows =
    batterVsPitcherCareerCache.get(
      careerMatchupKey
    );
...
if (
  Array.isArray(careerMatchupRows) &&
  careerMatchupRows.length
) {
  const careerMatchupData =
    buildBatterVsPitcherCareerData({
  ...
  if (
    careerMatchupData?.coverage?.percentage !== null &&
    careerMatchupData?.coverage?.percentage !== undefined
  ) {
...
function buildPlayerPropCareerParkCoverage({
  logs = [],
  market,
  line,
  side,
  ...
  /*
   * Si no sabemos cuál es el parque
   * actual, no fabricamos información.
   */
  if (
    !currentVenueId &&
    !currentVenueName
...
function buildPlayerPropCareerOpponentCoverage({
  logs = [],
  market,
  line,
  side,
  ...
  /*
   * SIN HISTORIAL.
   * No usamos temporada actual,
   * otro rival ni ningún fallback.
   */
  if (!eligibleLogs.length) {
    return null;
  }
...
function extractBatterVsPitcherFromPlayByPlay(
  data,
  batterId,
  pitcherId
) {
...
async function getBatterVsPitcherCareerGames(
  ...
  /*
   * Las carreras completas ya pasan
   * por nuestro caché por temporada.
   */
  const [
    batterLogs,
    pitcherLogs
  ...
  /*
   * gamePk del pitcher.
   * No exigimos que haya sido abridor:
   * puede haberse enfrentado al bateador
   * como relevista.
   */
  ...
  /*
   * Juegos donde AMBOS participaron.
   */
  const candidateGames =
    batterLogs.filter(gameLog => {
  ...
  /*
   * Leemos todos los juegos ya
   * procesados para este matchup.
   */
  const {
    data: cachedRows,
    error: cacheError
  ...
  /*
   * Solo partidos que todavía
   * NO existen en Supabase.
   */
  const missingGames =
    candidateGames.filter(gameLog => {
   ...
   * Evita disparar 20-30 requests
   * simultáneos en Vercel.
   */
  for (
    let i = 0;
    i < missingGames.length;
    i += 5
  ) {
    ...
    await Promise.all(
      batch.map(async gameLog => {
        ...
        try {
          ...
          /*
           * Si MLB falla NO guardamos
           * faced=false porque podría
           * ser solamente un error temporal.
           */
          if (!response.ok) {
            return;
          }
          ...
          const matchup =
            extractBatterVsPitcherFromPlayByPlay(
              data,
              batterId,
              pitcherId
            );
          ...
          const row = {
            ...
            /*
             * Guardamos también FALSE.
             * Así sabemos que ya revisamos
             * ese partido y nunca tenemos
             * que descargarlo otra vez.
             */
            faced:
              matchup.faced,
           ...
           * para devolverlo inmediatamente.
           */
          cacheByGame.set(
            gamePk,
            {
  ...
  /*
   * Devolvemos únicamente partidos
   * donde realmente se enfrentaron.
   */
const batterLogByGame =
  new Map(
    batterLogs.map(gameLog => [
...
function buildBatterVsPitcherCareerData({
  rows = [],
  market,
  line,
  side
...
const pitcherId =
  Number(
    req.query.pitcherId ||
    req.query.opponentPitcherId ||
    0
  );

const pitcherName =
  String(
    req.query.pitcherName ||
    req.query.opponentPitcher ||
    contextValue ||
    ""
  );
