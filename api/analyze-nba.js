...
// ============================================================
// LEGACY ONESIGNAL PREMIUM ALERTS — DISABLED
//
// Este sender antiguo enviaba directamente al segmento
// "Active Subscriptions" de OneSignal.
//
// Las notificaciones nuevas serán gestionadas únicamente por
// Market Intelligence:
// outbox -> revalidation -> fanout -> delivery -> provider.
//
// Se devuelve 200 para que cualquier cron antiguo que todavía
// invoque este endpoint no genere errores ni reintentos.
// ============================================================

if (
  req.method === "GET" &&
  req.query.mode === "send-premium-alerts"
) {
  return res.status(200).json({
    ok: true,
    disabled: true,
    sent: 0,
    message: "Legacy OneSignal premium alerts are disabled."
  });
}
...
const total = Number(analysis.premium.totalLine || 0);
...
async function saveParlayTracking(gameDate, picks) {
  if (!picks?.length) return;

  const legOddsDefault = 1.91;

  const parlayOdds = picks.reduce((total, leg) => {
    const odds = Number(leg.odds_decimal || legOddsDefault);
    return total * odds;
  }, 1);

  const averageLegOdds =
    picks.length > 0
      ? picks.reduce((sum, leg) => sum + Number(leg.odds_decimal || legOddsDefault), 0) / picks.length
      : legOddsDefault;
  ...
  if (!savedTrackingParlay?.id) return;
  ...
  const legRows = picks.map(leg => ({
    ...
    odds_decimal: Number(leg.odds_decimal || legOddsDefault),
    result: "pending"
  }));
