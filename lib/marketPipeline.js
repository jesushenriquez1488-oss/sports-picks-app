...
let marketReadFinishedAt =
  null;

let importantFinishedAt =
  null;

let notificationFinishedAt =
  null;
...
// 3. MARKET READ
//
// Human interpretation of the CURRENT complete market state.
//
// At this point:
//
// - Market Evaluation is current
// - Sharp Score V2 is current
// - Opportunity is current
//
// Market Read does NOT modify CashEdge Confidence.
// ========================================================

const marketReadStartedMs =
  Date.now();
...
marketReadFinishedAt =
  nowIso();
    ...
    // IMPORTANT:
    //
    // This stage decides whether the current market state
    // deserves a notification candidate.
    //
    // It does NOT send a push.
    //
    // With notification rules disabled, it returns safely
    // without creating an outbox row.
    //
    // Later, rules may be enabled while
    // notifications_enabled=false so candidates are stored
    // with status="shadow" and dispatch_enabled=false.
    //
    // Notification-decision failures are intentionally
    // NON-FATAL. A notification problem must never interrupt
    // the core Market Intelligence pipeline.
    // ========================================================

    const notificationStartedMs =
      Date.now();
    ...
    try {
      ...
      notificationResult =
        await evaluateNotificationDecision({
      ...
      console.error(
        "MARKET NOTIFICATION DECISION ERROR:",
    ...
    notificationMs =
      Date.now() -
      notificationStartedMs;


    notificationFinishedAt =
      nowIso();
    ...
    // PERFORMANCE LOG
    //
    // No new DB columns are required for Notification Decision
    // at this stage.
    //
    // total_ms includes the complete processing pass.
    //
    // received_to_important_ms intentionally retains its
    // original meaning and stops at Important Moves.
    // ========================================================
