...
function isStrongSharpWithCashEdgeEvent(
  ...
  // There must be an actual market move.
  ...
  // Strong Sharp push requires broad market confirmation.
  if (
    referenceAligned < 1 ||
    alignedBooks < 3
  ) {
...
function classifyNotification({
  ...
  const footballSpreadMateriality =
  ...
  //
  // NFL / NCAAF spreads use the dedicated football rules.
  //
  // Other sports / markets temporarily keep the existing
  // material-value rules until their notification thresholds
  // are migrated individually.
  // ==========================================================

  const opportunityIsMaterial =
    footballSpreadMateriality
      .applies === true
      ? footballSpreadMateriality
          .qualifies === true
      : isMaterialValue(
  ...
  //
  // This remains only until Strong Sharp + Reversal replace
  // MATERIAL_MOVE in the notification policy.
  // ==========================================================
...
async function evaluateNotificationDecision({
  ...
  // RULE DISABLED
  //
  // While enabled=false nothing enters the outbox.
  // ==========================================================
  ...
  // FINGERPRINT
  //
  // OPPORTUNITY_DETECTED is deduplicated by MATERIAL MARKET
  // STATE, not by the opportunity event id.
  //
  // This allows a new alert after the cooldown when the market
  // has genuinely taken another step, while preventing ordinary
  // refreshes / minor price ticks from creating duplicates.
  // ==========================================================
  ...
  // Price-only opportunities move in material 10-cent steps.
  // Example:
  // 10 -> 12 cents remains the same notification state.
  // 10 -> 20 cents becomes a new material state.
  ...
  const fingerprint =
    kind ===
      ...
      ? [
          ...
          // Only material price-value steps matter.
