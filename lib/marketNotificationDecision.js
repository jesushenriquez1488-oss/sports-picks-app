...
// ============================================================
// NOTIFICATION GAME WINDOW
//
// Push notifications are only actionable when the game is:
// - still pregame
// - 24 hours away or less
//
// Market Intelligence continues monitoring games normally.
// This restriction applies ONLY to push notifications.
//
// Source priority:
// 1. market_provider_events.game_time
// 2. daily_picks.game_time
// ============================================================

async function getNotificationGameWindow({
  ...
  // FAIL CLOSED
  //
  // If CashEdge cannot verify the start time, do not send.
  // ==========================================================
  ...
  // Too early for an actionable push.
  if (
    minutesToStart >
    maxMinutes
  ) {
    ...
    return {
      ...
      minutesToStart,
      reason:
        `Game is more than ${maxHours} hours away`
    };
  }
  ...
  return {
    ...
    minutesToStart,
    reason:
      "Game is inside notification window"
  };
}
...
// ============================================================
// STRONG SHARP AGAINST CASHEDGE — REVERSAL MATERIALITY
//
// MARKET_REVERSAL requires:
//
// 1. Real Strong Sharp conflict state.
// 2. Direction AGAINST CashEdge.
// 3. Actual sportsbook movement AGAINST CashEdge.
// 4. Multiple sportsbooks confirming the move.
// 5. At least one reference sportsbook participating.
//
// Whether a specific user previously received the WITH alert
// is enforced later during fanout, per recipient.
// ============================================================

function isStrongSharpAgainstCashEdgeEvent(
  event
) {
...
// ============================================================
// OPPORTUNITY PUSH STATE SAFETY
//
// OPPORTUNITY_DETECTED may only exist while the canonical
// Opportunity State still represents a real actionable window.
//
// IMPORTANT:
//
// VALUE_AVAILABLE  = valid opportunity
// WINDOW_CLOSING   = still valid, but deteriorating
//
// MARKET_SPLIT     = NO CONSENSUS -> never send opportunity push
// MARKET_CONFLICT  = market against CashEdge -> never send
// OPPORTUNITY_CLOSED = opportunity ended -> never send
// NO_VALUE / NO_DATA = never send
//
// We also explicitly block the live split_market condition,
// even if the persisted state has not caught up yet.
// ============================================================

function isOpportunityNotificationStateEligible({
  opportunityResult,
  summary
}) {
  ...
  // Current market has no reliable consensus.
  // Never send OPPORTUNITY_DETECTED from a split market.
  if (
    opportunityType ===
    "SPLIT_MARKET"
  ) {
...
//
// Strong Sharp and Market Reversal remain independent from
// Opportunity state.
//
// OPPORTUNITY_DETECTED requires BOTH:
// 1. notification materiality
// 2. a currently valid Opportunity State
// ============================================================
...
function classifyNotificationCandidates({
  ...
  // 2. MARKET REVERSAL
  //
  // Recipient-level proof of a previously SENT Strong Sharp
  // WITH CashEdge is enforced later in fanout.
  // ==========================================================

  if (
    isStrongSharpAgainstCashEdgeEvent(
      latestActiveSignalEvent
    )
  ) {
  ...
  const opportunityStateEligible =
    isOpportunityNotificationStateEligible({
...
//
// Used again during notification revalidation.
//
// This MUST use the same Opportunity State safety rule so an
// alert created earlier cannot survive if the market changes
// to split / conflict / closed before fanout.
// ============================================================
...
function buildNotificationContent({
  ...
  if (
    ...
    return {
      ...
      body:
        `${pick}. Strong sharp action has reversed and is now confirmed against the CashEdge side by multiple sportsbooks. Open Premium Radar for the latest market state.`,
...
//
// Do not create ANY immediate notification more than
// 24 hours before game start.
// ==========================================================

const notificationGameWindow =
  await getNotificationGameWindow({
  ...
  for (
    ...
    if (
      ...
      candidateResults.push({
        ...
        skipped: true,
        reason:
          "Strong Sharp reversal condition is no longer valid"
      });
