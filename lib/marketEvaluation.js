...
// ==========================================================
// MOVEMENT REFERENCE MARKET
//
// MLB Runline remains the actual CashEdge wager,
// but team-side market movement is measured with Moneyline.
// ==========================================================

const useMLBMoneylineReference =
  normalizeText(context.sport) === "mlb" &&
  marketType === "spread";


const movementMarketType =
  useMLBMoneylineReference
    ? normalizeText(
        context.movement_reference_market_type ||
        "moneyline"
      )
    : marketType;
...
const movementSelectionKey =
  useMLBMoneylineReference
    ? normalizeText(
        context.movement_reference_selection_key ||
        selectionKey
      )
    : selectionKey;
...
//
// For MLB Runline:
// freeze the first observed Moneyline consensus and NEVER
// overwrite it. Future movement is measured from this point.
//
// All other markets continue using the original CashEdge
// Premium baseline.
// ==========================================================

let movementBaselinePrice =
  useMLBMoneylineReference
    ? safeNum(
        context
...
if (
  useMLBMoneylineReference &&
  movementBaselinePrice === null &&
  consensus?.status === "clear"
) {
  ...
  if (
    ...
    const {
      ...
      await supabaseAdmin
        ...
        .update({
          ...
          movement_baseline_implied_probability:
            capturedProbability,

          movement_baseline_captured_at:
            capturedAt
        })
        .eq(
          "cashedge_game_id",
    ...
    if (capturedBaseline) {
      ...
      /*
       * Race protection:
       * another evaluation may have frozen the baseline first.
       */

      const {
        data: existingBaseline,
        error: existingBaselineError
      } =
        await supabaseAdmin
          .from(
            "market_pick_context"
      ...
      if (existingBaselineError) {
        throw existingBaselineError;
      }
...
const alignment =
 ...
 // ==========================================================
// MONEY / TICKETS — LATEST PER SPLIT SOURCE
//
// IMPORTANT:
// Never average percentages from different sportsbooks.
// Each sportsbook remains independent.
// ==========================================================
...
// ==========================================================
// KEEP ONLY THE LATEST ROW FROM EACH SPORTSBOOK
// ==========================================================
...
// MULTI-SOURCE STATE
//
// We do NOT manufacture one percentage from several books.
//
// 0 sources:
//   NO_DATA
//
// 1 source:
//   use that source as current split
//
// 2+ sources:
//   keep percentages separate.
//   If their directional readings differ, MIXED.
// ==========================================================
...
//
// Only expose one money/tickets pair when there is exactly
// one sportsbook source.
//
// Multiple books must never be collapsed into one percentage.
// ==========================================================
...
const divergence =
  ...
  // GAME TIME
  //
  // Prefer provider mapping when available.
  // Fallback to canonical CashEdge daily_picks.game_time.
  // ==========================================================
  ...
  // ==========================================================
  // FALLBACK — CASHEDGE CANONICAL GAME TIME
  // ==========================================================
...
movementBaselinePriceAmerican:
  movementBaselinePrice,

movementBaselineCapturedAt:
  context
    .movement_baseline_captured_at ||
  null,
          thresholds: {
